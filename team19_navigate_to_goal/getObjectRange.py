...
class ObjectRangePubsub(Node):
	def __init__(self):
		...
		self.obj_dist = self.create_publisher(Float32, 'obj_dist', 10)
	...
	def _lidar_callback(self,msg):
		...
		if len(lidar_data) > 200:
			...
			theta1 = 0.0
			theta2 = 20.0 # right edge
			theta3 = 340.0 # left edge
			theta4 = 360.0 # center
			...
			self.get_logger().info('frontLeft: {}'.format(len(frontLeft)))
			...
			self.get_logger().info('minFrontData{}'.format(minFrontData))
